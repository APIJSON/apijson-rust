...
pub async fn handle_get(manager: &DataSourceManager, body_map: HashMap<String, serde_json::Value>) -> RpcResult::<HashMap<String, serde_json::Value>> {
    ...
    let defaults = DatabaseTargetDefaults::new(
        manager.get_datasource_names().first().cloned(),
        manager.get_datasource_names().first()
            .map(|ds_name| manager.get_database_names(&ds_name))
            .and_then(|db_names| db_names.first().cloned()),
        Some("public".to_string())
    );
    
    // 获取数据源类型
    let datasource_name = if !parse_result.items.is_empty() {
        let target = defaults.apply_defaults(parse_result.items[0].target.clone());
        target.datasource.unwrap_or_else(|| "default".to_string())
    } else {
        manager.get_datasource_names().first().cloned().unwrap_or_else(|| "default".to_string())
    };
    
    let datasource_kind = manager.get_datasource_config(&datasource_name)
        .map(|config| config.kind.clone())
        .unwrap_or(DataSourceKind::Mysql);
    
    let mut ctx = QueryContext::from_json(body_map, datasource_kind, Some(defaults));
    ctx.response(manager).await
}
...
impl QueryContext {
    ...
    async fn query_primary_node(&mut self, node: &mut QueryNode, manager: &DataSourceManager) {
        // 添加关联字段到查询列
        ...
        // 查询主节点数据
        let Some(node_results) = self.query_node_data(node, manager).await else { return };
